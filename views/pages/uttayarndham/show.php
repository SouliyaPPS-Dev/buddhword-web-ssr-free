<div x-data="{
    ...
    get favIndex() {
        return this.favorites.findIndex(f => f.id === '<?= htmlspecialchars($url) ?>');
    },
    get isInFavorites() {
    ...
    toggleFavorite() {
        try {
            ...
            if (this.isInFavorites) {
                favs = favs.filter(f => f.id !== '<?= htmlspecialchars($url) ?>');
            } else {
                favs.push({ id: '<?= htmlspecialchars($url) ?>', title: '<?= htmlspecialchars($title) ?>', category: 'uttayarndham' });
            }
